...
int init_ctx(struct ctx_s * ctx, size_t stack_size, funct_t f, void * arg) {
	...
	/**
	 * Explore the ctx list to get a free core
	 */
	temp = *ctx;
	while (temp != *ctx) {
		cores[temp->ctx_core_id] = 1;
	}
...
void switch_to_ctx(struct ctx_s * ctx) {
	...
	while (ctx->ctx_core_id == core_id
			&& (ctx->ctx_state == CTX_END || ctx->ctx_state == CTX_STOP)) {
		if (ctx_ring == ctx) {
